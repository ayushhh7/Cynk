...
object YouTube {
    ...
    suspend fun getChartsPage(continuation: String? = null): Result<ChartsPage> = runCatching {
        ...
        sectionListRenderer?.contents?.forEach { content ->
            content.musicCarouselShelfRenderer?.let { renderer ->
                val carouselTitle = renderer.header?.musicCarouselShelfBasicHeaderRenderer?.title?.runs?.firstOrNull()?.text
                    ?: "Charts"

                val directSongItems = mutableListOf<SongItem>()
                val playlistSections = mutableListOf<ChartsPage.ChartSection>()

                renderer.contents.forEach { item ->
                    item.musicResponsiveListItemRenderer?.let { r ->
                        val chartItem = convertToChartItem(r)
                        if (chartItem is SongItem) {
                            directSongItems.add(chartItem)
                        }
                    }
                    item.musicTwoRowItemRenderer?.let { r ->
                        if (r.isSong) {
                            val song = convertMusicTwoRowItem(r)
                            if (song is SongItem) {
                                directSongItems.add(song)
                            }
                        } else if (r.isPlaylist) {
                            val playlistId = r.navigationEndpoint.browseEndpoint?.browseId?.removePrefix("VL")
                                ?: r.thumbnailOverlay?.musicItemThumbnailOverlayRenderer?.content
                                    ?.musicPlayButtonRenderer?.playNavigationEndpoint
                                    ?.watchPlaylistEndpoint?.playlistId
                            val itemTitle = r.title.runs?.firstOrNull()?.text ?: "Chart"
                            if (playlistId != null) {
                                val songs = playlist(playlistId).getOrNull()?.songs.orEmpty()
                                if (songs.isNotEmpty()) {
                                    playlistSections.add(
                                        ChartsPage.ChartSection(
                                            title = itemTitle,
                                            items = songs,
                                            chartType = determineChartType(itemTitle)
                                        )
                                    )
                                }
                            }
                ...
                if (directSongItems.isNotEmpty()) {
                    sections.add(
                        ChartsPage.ChartSection(
                            title = carouselTitle,
                            items = directSongItems,
                            chartType = determineChartType(carouselTitle)
                        )
                    )
                }
                sections.addAll(playlistSections)
            }

            content.musicShelfRenderer?.let { renderer ->
                val title = renderer.title?.runs?.firstOrNull()?.text
                val songs = renderer.contents?.mapNotNull { item ->
                    item.musicResponsiveListItemRenderer?.let { convertToChartItem(it) as? SongItem }
                }.orEmpty()
                if (title != null && songs.isNotEmpty()) {
                    sections.add(
                        ChartsPage.ChartSection(
                            title = title,
                            items = songs,
        ...
        // Fallback: If no chart sections were extracted, fetch the default trending playlist
        if (sections.isEmpty()) {
            val trendingPlaylistId = "OLAK5uy_lSTp1DIuzZBUyee3kDsXwPgP25WdfwB40"
            val fallbackSongs = playlist(trendingPlaylistId).getOrNull()?.songs.orEmpty()
            if (fallbackSongs.isNotEmpty()) {
                sections.add(
                    ChartsPage.ChartSection(
                        title = "Trending",
                        items = fallbackSongs,
                        chartType = ChartsPage.ChartType.TRENDING
                    )
                )
            }
        }
    ...
    private fun convertToChartItem(renderer: MusicResponsiveListItemRenderer): YTItem? {
        return try {
            ...
            val firstColumn = renderer.flexColumns.firstOrNull()
            ...
            val title = firstColumn.runs?.firstOrNull()?.text?.takeIf { it.isNotBlank() } ?: return null
