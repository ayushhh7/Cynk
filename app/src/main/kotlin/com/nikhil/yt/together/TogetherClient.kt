...
sealed interface TogetherClientEvent {
    ...
    data class JoinDecision(
        val decision: com.nikhil.yt.together.JoinDecision,
    ) : TogetherClientEvent
    ...
    data class HeartbeatPong(
        val pong: com.nikhil.yt.together.HeartbeatPong,
...
class TogetherClient(
    ...
    var onEvent: ((TogetherClientEvent) -> Unit)? = null

    private val _events = MutableSharedFlow<TogetherClientEvent>(extraBufferCapacity = 64)
    val events = _events.asSharedFlow()

    private fun dispatchEvent(event: TogetherClientEvent) {
    ...
    private var lastJoinInfo: TogetherJoinInfo? = null
    private var lastDisplayName: String = ""
    private var lastRemoteWsUrl: String? = null
    private var lastRemoteSessionId: String? = null
    private var lastRemoteSessionKey: String? = null

    fun connect(joinInfo: TogetherJoinInfo, displayName: String) {
        isExplicitDisconnect = false
        lastJoinInfo = joinInfo
        lastDisplayName = displayName
        lastRemoteWsUrl = null
    ...
    fun connect(
        ...
        isExplicitDisconnect = false
        lastRemoteWsUrl = wsUrl
        lastRemoteSessionId = sessionId
        lastRemoteSessionKey = sessionKey
        lastDisplayName = displayName
        lastJoinInfo = null
                    ...
                    while (true) {
                        ...
                        val message =
                            runCatching { TogetherJson.json.decodeFromString(TogetherMessage.serializer(), text) }
                                .getOrElse {
                                    dispatchEvent(TogetherClientEvent.Error("Failed to decode message", it))
                                    continue
                                }
                        ...
                        when (message) {
                            is ServerWelcome -> {
                                if (message.sessionId == sessionId) {
                                    ...
                                    dispatchEvent(TogetherClientEvent.Welcome(message))
                                }
                            }

                            is RoomStateMessage -> {
                                if (message.state.sessionId == sessionId) {
                                    dispatchEvent(TogetherClientEvent.RoomState(message.state))
                                }
                            }

                            is com.nikhil.yt.together.JoinDecision -> {
                                if (message.sessionId == sessionId && message.participantId == selfParticipantId) {
                                    dispatchEvent(TogetherClientEvent.JoinDecision(message))
                                }
                            }

                            is ParticipantJoined -> {
                                if (message.sessionId == sessionId) {
                                    dispatchEvent(TogetherClientEvent.ParticipantJoined(message.participant))
                                }
                            }

                            is ParticipantLeft -> {
                                if (message.sessionId == sessionId) {
                                    dispatchEvent(TogetherClientEvent.ParticipantLeft(message.participantId, message.reason))
                                }
                            }
                            ...
                            is KickParticipant -> {
                                if (message.sessionId == sessionId && message.participantId == selfParticipantId) {
                                    ...
                                    dispatchEvent(TogetherClientEvent.Error(detail, null))
                                    break
                                }
                            }
                                ...
                                if (message.sessionId == sessionId) {
                                    dispatchEvent(
                                        TogetherClientEvent.HeartbeatPong(
                                            pong = message,
                                            receivedAtElapsedRealtimeMs = System.currentTimeMillis(),
                                        ),
                                    )
                                }
                            }
                            ...
                            is ServerError -> {
                                if (message.code == "SESSION_NOT_FOUND" || message.code == "BANNED") {
                                    isExplicitDisconnect = true
                                }
                                dispatchEvent(TogetherClientEvent.ServerIssue(message = message.message, code = message.code))
                            }
    ...
    private fun scheduleReconnect() {
        val remoteWs = lastRemoteWsUrl
        val remoteSid = lastRemoteSessionId
        val remoteKey = lastRemoteSessionKey
        ...
        reconnectJob =
            scope.launch {
                ...
                while (attempt < 4 && !isExplicitDisconnect) {
                    attempt++
                    _state.value = TogetherClientState.Reconnecting(wsUrl = remoteWs, sessionId = remoteSid, attempt = attempt)
                    dispatchEvent(TogetherClientEvent.Reconnecting(attempt = attempt))
                    val delayMs = (1000L * (1 shl (attempt - 1))).coerceAtMost(6000L)
                    kotlinx.coroutines.delay(delayMs)
                    if (isExplicitDisconnect) break

                    val token = normalizedBearerToken
                    val urls = listOfNotNull(remoteWs.trim(), alternateWebSocketSchemeOrNull(remoteWs.trim())).distinct()
                    for (candidate in urls) {
                        try {
                            client.webSocket(
                                urlString = candidate,
                                ...
                                send(TogetherJson.json.encodeToString(TogetherMessage.serializer(), hello))
                                _state.value = TogetherClientState.ConnectedRemote(wsUrl = candidate, sessionId = remoteSid)
