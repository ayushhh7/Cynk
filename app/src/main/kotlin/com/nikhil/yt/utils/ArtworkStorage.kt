...
object ArtworkStorage {
    private const val FILENAME = "cynk_saved_artworks.json"
    private const val LEGACY_FILENAME = "velune_saved_artworks.json"
    private val json = Json { prettyPrint = true; ignoreUnknownKeys = true }

    private fun fileFor(context: Context): File {
        val current = File(context.filesDir, FILENAME)
        if (!current.exists()) {
            val legacy = File(context.filesDir, LEGACY_FILENAME)
            if (legacy.exists()) {
                runCatching {
